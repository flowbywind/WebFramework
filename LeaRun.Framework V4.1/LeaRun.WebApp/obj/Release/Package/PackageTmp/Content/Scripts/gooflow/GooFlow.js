...
function GooFlow(bgDiv, property) {
    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            var NodeId = (Number($('.GooFlow_item').length)+1);
            //var NodeId = e.data.inthis.$max;
            e.data.inthis.addNode(e.data.inthis.$id + "_node_" + NodeId, { name: "node_" + NodeId, left: X, top: Y, type: e.data.inthis.$nowType });
            e.data.inthis.$max++;
        });
        //划线时用的绑定
        ...
        $(window).keydown({ inthis: this }, function (e) {
            //绑定键盘操作
            var This = e.data.inthis;
...
GooFlow.prototype = {
    ...
    initGroup: function (width, height) {
        ...
        //绑定点击事件
        this.$group.click({ inthis: this }, function (e) {
            var This = e.data.inthis;
            if (This.$nowType != "group") return;
            if (!e) e = window.event;
            switch ($(e.target).attr("class")) {
    ...
    addNode: function (id, json) {
        ...
        if (json.type.indexOf(" round") < 0) {
            ...
            this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' divtype='" + json.type + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + (json.width - 2 + 20) + "px;height:" + (json.height - 2 + 18) + "px;'><tr><td class='ico'><b class='ico_" + json.type + "'></b></td><td>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
            if (json.type.indexOf(" mix") > -1) this.$nodeDom[id].addClass("item_mix");
        }
        else {
            json.width = 24; json.height = 24;
            this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' divtype='" + json.type + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='0'><tr><td class='ico'><b class='ico_" + json.type + "'></b></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>" + json.name + "</div></div>");
        }
